...
const ForeCast = ({ data }: Props): JSX.Element => {
  ...
  return (
    <div className="w-full md:max-w-[500px] h-full p-4 flex flex-col text-center items-center justify-center md:px-10 lg:p-24 lg:h-[800px] bg-white bg-opacity-20 backdrop-blur-lg drop-shadow-lg rounded text-black">
      <div className="mx-auto w-[300px]">
        ...
        <section className='flex overflow-x-scroll mt-4 pb-2 mb-5'>
          {data.list.map((item, i) => (
            <div className='inline-block text-center w-[50px] flex-shrink-0' key={i}>
                ...
                <p><Degree temp={Math.floor(today.main.temp)} /></p>
            </div>
          ))}
        </section>
        <section className='flex flex-wrap justify-between text-zinc-700'>
            ...
            <Tile icon = "visibility" title = "Visisbility" info = {`${(today.visibility / 1000).toFixed()}km`} description = {getVisibilityValue(today.visibility)} />
        </section>
      </div>
    </div>
  )
